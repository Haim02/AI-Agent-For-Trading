...
function StockCard({ stock }: { stock: Stock }) {
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="bg-slate-950 px-2" dir="ltr">
        {!imgError ? (
          <img
            src={stock.chart_url}
      ...
      {stock.news?.length > 0 && (
        <div className="px-4 py-3">
          ...
          <div className="space-y-1.5">
            {stock.news.map((n, i) => (
              <div key={i} className="flex gap-2 text-sm leading-snug text-slate-300">
      ...
      <div className="flex gap-2 border-t border-slate-800 px-4 py-2.5">
        <a
          href={`https://finviz.com/quote.ashx?t=${stock.ticker}`}
          target="_blank"
          rel="noopener noreferrer"
          className="flex-1 rounded-lg bg-slate-800 py-2 text-center text-xs font-medium text-slate-300"
        >
          FinViz
        </a>
        <a
...
export default function Scanner() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-24 text-white">
      ...
      {isLoading && (
        <div className="py-12 text-center text-slate-500">טוען...</div>
      )}

      {!isLoading && stocks.length === 0 && (
        <div className="py-16 text-center">
          <div className="mb-4 text-5xl">🔍</div>
          <div className="mb-4 text-slate-400">אין תוצאות סריקה עדיין</div>
          <button
            onClick={handleScan}
            disabled={scanning}
